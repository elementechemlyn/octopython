...
class Products(object):
    def __init__(self,is_variable:bool=None,is_green:bool=None,is_tracker:bool=None,is_prepay:bool=None,is_business:bool=False,available_at:datetime=None,api_key:str=None):
    ...
    def _call_products(self) -> None:
        for status,products in self._client.get_products(self.is_variable,self.is_green,self.is_tracker,self.is_prepay,self.is_business,self.available_at):
            self._count = 0
            self._products = []
            self.status = status
            if status==200:
                ...
                for product in products['results']:
                    ...
                    p = dataclasses.ProductsData(
                        ...
                        product['term'],
                        product['brand'],
                        None if product['available_from']==None else utils.from_iso(product['available_from']),
                        None if product['available_to']==None else utils.from_iso(product['available_to']),
                        links
                    )
...
class Product(object):
    ...
    def _build_tariffs(self,tariff:dict) -> dict[str:dataclasses.Tariff]:
        ...
        for gsp,details in tariff.items():
            direct_debit_monthly_tariff = self._build_tariff_detail(details.get('direct_debit_monthly'))
            direct_debit_quarterly_tariff = self._build_tariff_detail(details.get('direct_debit_quarterly')) 
            t = dataclasses.Tariff(gsp,direct_debit_monthly_tariff,direct_debit_quarterly_tariff)
            tariffs[gsp] = t
        return tariffs            
...
class MeterPoint(object):
    ...
    def _call_meter_point(self) -> None:
        ...
        self.status = status
        if status==200:
            mpd = dataclasses.MeterPointData(meter_point_json['mpan'],meter_point_json['gsp'],meter_point_json['profile_class'])
